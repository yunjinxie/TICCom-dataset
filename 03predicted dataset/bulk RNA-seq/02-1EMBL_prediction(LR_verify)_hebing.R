#==================================整合EMBL=====================================
setwd("I:/F/tumor_immune_interaction/computation/A_webserver/EMBL_expression_atlas/30_log_05/result")

d <- dir(getwd())
cancer <- d[!grepl(".txt",d)]
...
write.table(cbind("Gene1Name","Gene2Name","cancer","Interaction_Strength","P_value"),
            paste0(getwd(),"/EMBL_interaction_strength_gene_result.txt"),sep='\t',quote = F,row.names = F,
...
for(i in 1:length(cancer)){
  ...
  write.table(data,paste0(getwd(),"/EMBL_interaction_strength_gene_result.txt"),sep='\t',quote = F,row.names = F,
              col.names = F,append = T)
}
...
data <- read.table(paste0(getwd(),"/EMBL_interaction_strength_gene_result.txt"),sep='\t',header = T,as.is = T,fill=T)
...
data_merge2 <- data_merge %>% select(Gene1Name = Gene1Name.x,Gene1Id,Cell,Gene2Name = Gene2Name.x,Gene2Id,Cell_1,
  ...
  filter(as.numeric(P_value) < 0.1)
head(data_merge2)

write.table(data_merge2,paste0(getwd(),"/search3_EMBL_verify.txt"),sep='\t',quote = F,row.names = F)
####配体-受体####
file <- "ICCom_gene_re.txt"
library(jsonlite)
library(dplyr)
write.table(cbind("ligand","receptor","cancer","Interaction_Strength","P_value","Funcion"),
            paste0(getwd(),"/EMBL_ICCom_result.txt"),sep='\t',quote = F,row.names = F,
            col.names = F,append = T)
for(i in 1:length(cancer)){
  filePath <- paste0(getwd(),"/",cancer[i],"/",file)
  ...
  data <- data[[1]]
  data$Cancer <- cancer[i]
  write.table(data,paste0(getwd(),"/EMBL_ICCom_result.txt"),sep='\t',quote = F,row.names = F,
              col.names = F,append = T)
}
...
data <- read.table(paste0(getwd(),"/EMBL_ICCom_result.txt"),sep='\t',header = T,as.is = T)
...
write.table(data_merge2,paste0(getwd(),"/search1_EMBL_LR.txt"),sep='\t',quote = F,row.names = F)
